    ...
    <Menu :model="links" class="w-full p-2">
      <template #start>
        <span class="align-items-center inline-flex gap-1 px-2 py-2">
          ...
          <span class="text-xl font-medium">RECRUITER</span>
        </span>
      </template>
      <Divider />
      <template #item="{ item, props }">
        <RouterLink v-if="true" v-slot="{ href, navigate }" :to="item.route" custom>
          <a
            v-ripple
            :href="href"
            v-bind="props.action"
      ...
      <template #end>
        <div class="mt-4">
          <div
            v-ripple
            class="text-color hover:surface-200 relative flex w-full items-center overflow-hidden p-2 pl-3"
          >
            <Avatar
